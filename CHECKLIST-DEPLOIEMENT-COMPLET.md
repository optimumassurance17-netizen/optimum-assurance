...
- **Prisma** : le client est généré via **`@prisma/client`** dans `node_modules` ; **`npm ci` / `npm install`** lance **`postinstall` → `prisma generate`**. Migration baseline appliquée côté base utilisée ; **`prisma/migrations/migration_lock.toml`** versionné (`provider = "postgresql"`) — requis pour **`migrate deploy`** (prod / bases persistantes). Le job CI **e2e** applique le schéma avec **`prisma db push`** sur une base Postgres jetable. Scripts `db:sync-url-vercel`, `verify-supabase`, etc. documentés dans `DEPLOY.md`.
