  ...
  return (
    <>
      ...
      {open && (
        ...
        >
          <div className="flex items-center justify-between px-4 py-3 bg-[#C65D3B] text-white">
              ...
              >
                <div
                  className={`max-w-[85%] rounded-2xl px-4 py-2.5 text-sm ${
                    msg.role === "user"
                      ? "bg-[#C65D3B] text-white rounded-br-md"
                ...
                >
                  ...
                  {msg.role === "assistant" && /faq|questions fréquentes/i.test(msg.content) && (
                    <Link
                      href="/faq"
                      className="inline-block mt-2 text-[#C65D3B] font-medium hover:underline"
                    >
                      Voir la FAQ →
                    </Link>
                  )}
                  {msg.role === "assistant" && /devis|tarif|souscri/i.test(msg.content) && (
                    <div className="mt-2 flex flex-wrap gap-2">
                      <Link
                        href="/devis"
                        className="text-[#C65D3B] font-medium hover:underline"
                      >
                        Devis décennale →
                      </Link>
                      <Link
              ...
              <div className="flex flex-wrap gap-2">
                {SUGGESTIONS.map((s) => (
                  <button
                    ...
                    className="text-xs px-3 py-2 rounded-xl bg-[#FEF3F0] border border-[#C65D3B]/30 text-[#C65D3B] hover:bg-[#C65D3B] hover:text-white transition-all"
                  >
                    {s}
                  </button>
                ))}
              ...
              <div className="flex justify-start">
                <div className="bg-[#f5f5f5] rounded-2xl rounded-bl-md px-4 py-2.5">
                  <span className="inline-flex gap-1">
                    <span className="w-2 h-2 bg-[#C65D3B] rounded-full animate-bounce" style={{ animationDelay: "0ms" }} />
                    <span className="w-2 h-2 bg-[#C65D3B] rounded-full animate-bounce" style={{ animationDelay: "150ms" }} />
                    <span className="w-2 h-2 bg-[#C65D3B] rounded-full animate-bounce" style={{ animationDelay: "300ms" }} />
                  </span>
                </div>
              </div>
            )}
          ...
          <div className="p-3 border-t border-[#e5e5e5]">
            <div className="flex gap-2">
              ...
              <button
                ...
                className="bg-[#C65D3B] text-white px-4 py-2.5 rounded-xl font-semibold text-sm disabled:opacity-50 disabled:cursor-not-allowed hover:bg-[#B04F2F] transition-all"
              >
                Envoyer
              </button>
            </div>
            <p className="text-xs text-[#171717] mt-2 text-center">
              100 % en ligne — <a href={`mailto:${contactEmail}`} className="text-[#C65D3B] hover:underline">{contactEmail}</a>
            </p>
          </div>
        </div>
      )}
