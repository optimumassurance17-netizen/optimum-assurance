...
export const EMAIL_TEMPLATES = {
  ...
  devisDoAjoute: (raisonSociale: string, numero: string, primeAnnuelle: number) => ({
    subject: `Votre devis et conditions dommage ouvrage ${numero} sont disponibles - Optimum Assurance`,
    text: `Bonjour ${raisonSociale},\n\nVotre devis et conditions dommage ouvrage sont disponibles dans votre espace client.\n\nRéférence : ${numero}\nPrime annuelle : ${primeAnnuelle.toLocaleString("fr-FR")} €\n\nRèglement : uniquement par virement bancaire via Mollie (instructions sur la page de paiement). Attestation délivrée après encaissement et activation du contrat.\n\n${APP_URL}/espace-client\n\nCordialement,\nOptimum Assurance`,
    html: `<p>Bonjour ${raisonSociale},</p><p>Votre <strong>devis et conditions dommage ouvrage</strong> sont disponibles dans votre espace client.</p><p><strong>Référence :</strong> ${numero}<br><strong>Prime annuelle :</strong> ${primeAnnuelle.toLocaleString("fr-FR")} €</p><p><a href="${APP_URL}/espace-client" style="color:#2563eb;font-weight:bold;background:#eff6ff;padding:12px 24px;border-radius:8px;text-decoration:none;display:inline-block">Accéder à mon espace client</a></p><p>Règlement : <strong>uniquement virement bancaire (Mollie)</strong> — coordonnées et référence sur la page sécurisée. Attestation après réception du virement et activation du contrat.</p><p>Cordialement,<br>Optimum Assurance</p>`,
  }),
