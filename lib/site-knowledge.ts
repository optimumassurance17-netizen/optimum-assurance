...
- **Dommage ouvrage** : **/devis-dommage-ouvrage** (formulaire 5 étapes). Si le client est **connecté** et qu’une **première demande** est déjà enregistrée, la page oriente vers **/espace-client/questionnaire-do-etude** (questionnaire d’étude détaillé, prérempli depuis le premier envoi). Pages SEO : **/dommage-ouvrage/[slug]** (auto-construction, particulier, etc.).
...
- **Assurance titre** : **/assurance-titre** — formulaire d’étude pour transactions immobilières / refinancements / closings complexes ; **pas de tarif instantané**. Le visiteur décrit l’actif, l’opération, le montant, le calendrier et les risques identifiés (servitude, charge, fraude, empiètement, etc.). Le produit est présenté comme un **complément** aux vérifications notariales / juridiques, avec retour équipe en général sous **24 à 48 h ouvrées**. Après l’envoi, le prospect peut **activer son espace client**, compléter un **questionnaire d’étude détaillé** sur **/espace-client/assurance-titre** et **déposer ses pièces** dans la GED (titre de propriété, projet d’acte, état hypothécaire, plan cadastral, note du notaire).
...
## Parcours assurance titre
1. **/assurance-titre** — Formulaire en **1 page** : contact, profil du souscripteur, type d’opération, type de bien, adresse, montant, date cible, implication d’un prêteur, risques identifiés et message libre. Brouillon auto-sauvegardé localement sur l’appareil pendant la saisie.
2. Envoi → lead dédié + alerte e-mail équipe + accusé de réception prospect. **Aucun tarif affiché** ; étude manuelle du dossier.
3. Après l’envoi, le prospect peut **créer son espace client** ou se connecter, puis accéder à **/espace-client/assurance-titre** pour compléter le questionnaire d’étude (parties prenantes, calendrier, anomalies, contentieux, pièces disponibles).
4. Les pièces peuvent être déposées dans la **GED** de l’espace client (titre de propriété, projet d’acte, état hypothécaire, plan cadastral, note du notaire, etc.).
